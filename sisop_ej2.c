...
int main(int argc, char *argv[]){
    ...
    if (argc != 2)
    {
        puts("Error en el pasaje de parametros. Escriba '/sisop_ej2 -Help' sin las comillas para recibir ayuda");
        exit(1);
    }
    char *ayuda="-Help";    //Muestro la ayuda
    if (argc == 2 && strcmp(argv[1],ayuda) == 0) //Muestro la ayuda al usuario
    {
        printf("\nEste programa tiene la funcionalidad de trabajar con Fibonacci usando threads.");
        puts("Usted deberá pasar un número entero N como parámetro y este programa creará N threads. Cada thread calculará un valor diferente de Fibonacci");
        puts("Ej: El primer thread calculará el valor de Fibonacci de N, el segundo el de N-1, el tercero de N-2, así sucesivamente hasta el número 1");
        puts("Ejemplo de ejecucion: ./sisop_ej2 3");
        printf("\n");
        exit(3);
    }
    int res = atoi(argv[1]);
    printf("\nHa pasado: %d como parametro\n",res);
    count=res;
